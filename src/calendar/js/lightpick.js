	  ...
	  Lightpick = function(options) {
		...
		if (opts.footer) {
		  ...
		  if (opts.footer === true) {
			...
			//   '</button>';
			html +=
			  '<button type="button" class="lightpick__today-action">' +
			  opts.locale.buttons.today +
			  '</button>';
		  } else {
			html += opts.footer;
		  }
		...
		self._onMouseDown = function(e) {
		  ...
		  if (
			...
			self.reset();
		  } else if(target.classList.contains('lightpick__today-action')) {
			self.gotoToday();
			  ...
			  if (days > 0 && !target.classList.contains('is-disabled')) {
				...
				if (typeof opts.locale.pluralize === 'function') {
				  pluralText = opts.locale.pluralize.call(
					self,
					days,
					opts.tooltipNights? opts.locale.tooltipNightWord : opts.locale.tooltip
				  );
				}
